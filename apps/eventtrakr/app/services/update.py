...
from stonepi_update import Updater, normalize_repo
...
def _updater(db: Session | None = None) -> Updater:
    def get_repo() -> str:
        if env.stonepi_session_secret.strip():
            return getattr(env, "github_repo", "") or ""
        if db is None:
            return getattr(env, "github_repo", "") or ""
        return settings.get_value(db, "github_repo") or getattr(env, "github_repo", "") or ""
...
def repo_from_db(db: Session) -> str:
    if env.stonepi_session_secret.strip():
        return normalize_repo(getattr(env, "github_repo", "") or "")
    return normalize_repo(settings.get_value(db, "github_repo") or getattr(env, "github_repo", "") or "")
