...
def test_normalize_repo_accepts_url_or_slug():
    assert normalize_repo("owner/FileServe") == "owner/FileServe"
    assert normalize_repo("https://github.com/owner/FileServe.git") == "owner/FileServe"
...
def _write_release_zip(path: Path, version: str, nested: bool = True) -> Path:
    root = "owner-FileServe-abc123/" if nested else ""
    with zipfile.ZipFile(path, "w") as archive:
        ...
        archive.writestr(f"{root}requirements.txt", "flask\n")
    return path
