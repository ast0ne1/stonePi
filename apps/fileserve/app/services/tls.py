...
def _ensure_ca() -> tuple[x509.Certificate, rsa.RSAPrivateKey]:
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "FileServe Local CA"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "FileServe"),
        ]
    )
...
def _build_server_cert(
    ...
    key = _new_rsa_key()
    now = datetime.now(timezone.utc)
    cn = next((s for s in sans if s.endswith(".local")), "fileserve.local")
    subject = x509.Name(
        [
